...
import { X, ArrowRight, Link, Music, RotateCcw, LogIn, Check, Sparkles, User, Bookmark, Trash2, Play } from 'lucide-react';
import { loginWithSpotify, getSpotifyProfile } from '../../api/spotify.service';
...
const MusicWidget = ({ url, onUrlChange, isOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        setShowInput(!embedUrl && !isSpotify);
    ...
    const getEmbedUrl = (inputUrl) => {
        ...
        try {
            ...
            const ytMatch = inputUrl.match(youtubeRegex);
    ...
    const saveCurrentPlaylist = () => {
        ...
        setSavedPlaylists([newItem, ...savedPlaylists]);
    };
    ...
    const loadSavedPlaylist = (savedUrl) => {
        setInputValue(savedUrl);
        handleLoad(savedUrl);
    };

    const handleLoad = (urlToLoad = inputValue) => {
        if (!urlToLoad) return;

        const spotifyMatch = urlToLoad.match(/open\.spotify\.com\/(track|album|playlist|artist)\/([a-zA-Z0-9]+)/);

        let newIsSpotifyContent = false;

        if (spotifyMatch && spotifyToken && deviceId) {
            const [, type, id] = spotifyMatch;
            ...
            newIsSpotifyContent = true;
        } else {
            newIsSpotifyContent = urlToLoad.includes('spotify');
        }

        setIsSpotifyContent(newIsSpotifyContent);
    ...
    const profileImage = userProfile?.images?.length > 0
        ...
        <div className="music-widget-container">
            <div className="music-header">
                ...
                <div className="music-controls">
                    {!isFormMode && (
                        <button onClick={handleReset} className="control-btn" title="Change Link">
                            <RotateCcw size={16} />
                        </button>
                    )}
                ...
                <div className={`view-slider ${isFormMode && !isAnimating ? 'slide-in' : 'slide-out'}`}>
                    <div className="input-view">
                        ...
                        <div className="input-group">
                            <input
                                type="text"
                                placeholder="Paste Spotify, YouTube, SoundCloud..."
                                value={inputValue}
                            ...
                            <button
                                onClick={() => handleLoad()}
                                disabled={!inputValue}
                                className={`action-btn play ${inputValue ? 'active' : ''}`}
                            >
                                <ArrowRight size={18} />
                            </button>
                        </div>
                                ...
                                <div className="list-scroll">
                                    {savedPlaylists.map((item) => (
                                        <div key={item.id} className="saved-item" onClick={() => loadSavedPlaylist(item.url)}>
                                            ...
                                            <button className="delete-btn" onClick={(e) => removePlaylist(item.id, e)}>
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    ))}
                        ...
                        <div className="spotify-card">
                            {!spotifyToken ? (
                                <div className="spotify-content">
                                    <div className="spotify-icon-bg">
                                        <LogIn size={16} color="#1db954" />
                                    </div>
                                    <div className="spotify-text">
                                        <span className="spotify-title">Connect Spotify</span>
                                        <span className="spotify-desc">Unlock full playback & controls</span>
                                    </div>
                                    <button onClick={handleSpotifyLogin} className="btn-connect">
                                        Connect
                                    </button>
                                </div>
                            ) : (
                                <div className="spotify-content connected">
                                    <div className="spotify-profile-wrapper">
                                        {profileImage ? (
                                            ...
                                            <div className="profile-avatar-placeholder">
                                                {userProfile?.display_name ? userProfile.display_name[0].toUpperCase() : <User size={16} />}
                                            </div>
                                        )}
                                    ...
                                    <div className="spotify-text">
                                        <span className="spotify-title">
                                            {userProfile ? `Hi, ${userProfile.display_name}` : 'Spotify Active'}
                                        </span>
                                        <span className="spotify-desc">Connected</span>
                                    </div>
            ...
            <style>{`
                .music-widget-container {
                    position: fixed; bottom: 90px; left: 2rem; width: 620px;
                ...
                .icon-glow {
                    ...
                    background: linear-gradient(135deg, var(--primary-color, #8b5cf6), #6366f1);
                    border-radius: 10px;
                ...
                .action-btn {
                    width: 40px; border-radius: 12px; border: none;
                    display: flex; align-items: center; justify-content: center;
                    cursor: pointer; transition: all 0.2s;
                }
                .action-btn.save {
                ...
                .action-btn.play.active {
                    background: var(--primary-color, #8b5cf6); color: white; cursor: pointer;
                ...
                .spotify-icon-bg {
                    width: 42px; height: 42px; background: rgba(29, 185, 84, 0.15);
                    border-radius: 12px; display: flex; align-items: center; justify-content: center;
                }
                .spotify-icon-bg.success { background: #1db954; }
                ...
                .online-badge {
                    ...
                    width: 16px; height: 16px; background: #1db954;
                    border: 3px solid #252525; border-radius: 50%;
                    display: flex; align-items: center; justify-content: center;
                }
                ...
                @media (max-width: 768px) {
                    .music-widget-container {
                        left: 50%; transform: translateX(-50%); bottom: 85px; width: 92%;
