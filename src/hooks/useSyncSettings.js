...
export const useSyncSettings = (user, token, isMaintenance, setters) => {
    ...
    useEffect(() => {
        ...
        if (!user || user.isGuest || !currentToken || currentToken.startsWith('offline_token_') || isMaintenance) return;
            ...
            try {
                if (isFirstLoad.current) {
                    ...
                    if (cloudSettings) {
                        ...
                        if (cloudSettings.autoStartPomodoros !== undefined) setters.setAutoStart(cloudSettings.autoStartPomodoros);
